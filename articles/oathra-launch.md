...
これはAPIキー不要のローカルデモです。実電話には別途、電話・音声サービスの設定が必要です。上のコマンドはGitHubのv0.1.14配布版を使います。npmレジストリの版は0.1.0です。
