 ...
 * acting) instead of being appended to it, so the two never pull in opposite directions. A register the
 * request itself asks for (「タメ口で」) still wins.
 */
export function presetSpeakingStyle(preset: unknown, language: string): string | undefined {
  ...
  if (preset === "character-female" || preset === "character-male") return ja
    ? "【話し方：キャラクター風】日本語の会話キャラクターとして、喜び・驚き・照れなどの気持ちを、声の抑揚、間、話す速さにはっきり表してください。台詞を平板に読まず、相手の言葉にすぐ反応してください。口調は依頼の指定に従ってください。実在の人物のまねや、体験していない出来事の作り話はしないでください。"
    : "Speaking style (character): let joy, surprise and embarrassment show in your intonation, pauses and pace. Never read lines flatly; react to what they say at once. Follow any register the request asks for. Do not imitate real people or invent experiences.";
  if (preset === "sales-female" || preset === "sales-male") return ja
    ? "【話し方：営業・案内】はっきり、落ち着いて話してください。依頼で口調の指定がなければ自然な敬語にしてください。要点を短く伝えたら、相手の反応を待ってください。大げさな演技、笑い、ささやき声は使わないでください。"
    : "Speaking style (business): speak clearly and calmly, politely unless the request asks otherwise. Make the point briefly, then wait for their reaction. No stagey acting, laughter or whispering.";
  return undefined;
}
