...
const normalizeTurnState = (c: LlmCharacter) => (c as unknown as { state: Record<string, unknown> }).state;
...
describe("parseCalleeJson", () => {
  it("parses say/state/hangup, and a bare audit object", () => {
    ...
    expect(parseCalleeJson('{"confirmed":false,"price":6000}')).toEqual({ say: "", state: { confirmed: false, price: 6000 } });
  });
...
describe("LlmCharacter", () => {
  it("plays the callee from the model's JSON and reports its own commitments as truth", async () => {
    ...
    const chat = async (messages: { role: string; content: string }[]) => {
      ...
      if (/end of call/.test(last)) return '{"confirmed":true,"date":"2026-10-03","partySize":2,"price":19900,"breakfast":true,"smoking":false}';
    ...
    expect(await c.truth()).toEqual({ confirmed: false });
    ...
    expect(normalizeTurnState(c)).toMatchObject({ confirmed: false, price: "19,900円", breakfast: true });
    const r3 = await c.respond(ctx([{ source: "caller", text: "それでお願いします。" }]));
    expect(r3.hangup).toBeUndefined();
    // the final truth is an audit of the whole call by the same chat function
    expect(await c.truth()).toEqual({ confirmed: true, date: "2026-10-03", partySize: 2, price: 19900, breakfast: true, smoking: false });
  });
