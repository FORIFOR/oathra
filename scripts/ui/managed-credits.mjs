...
try {
 ...
 await page.click('#managed-logout');await page.until("!document.querySelector('#managed-login').hidden");assert.ok(!await page.visible('#managed-credit-button'));assert.equal(app.store.list('mission').length,0);assert.deepEqual(page.pageErrors,[]);
