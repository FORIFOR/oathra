...
try {
 ...
 await page.js("const submit=document.querySelector('#phone-form button[type=submit]');submit.click();submit.click()");await page.until("document.querySelector('#phone-review').open");assert.match(await page.text('#phone-review-fields'),/AIの声 vesper/);assert.equal((await(await req('/phone/history')).json())[0].request.voice,'vesper');assert.ok(await page.visible('#phone-setup'));assert.equal(await page.js("document.querySelector('#phone-dial').disabled"),true);assert.equal(await page.js("document.querySelector('#phone-consent')"),null);await page.press('Escape');assert.equal(await page.js("document.querySelector('#phone-review').open"),false);assert.equal((await page.focused()).id,'phone-instruction');
