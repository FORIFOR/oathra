...
const {billingConfiguration}=await import('../../apps/gateway/lib/billing.mjs');config.billing=billingConfiguration({OATHRA_CREDIT_POLICY:'provider-cost-v1',OATHRA_CREDIT_USD:'0.01',OATHRA_VOICE_ENGINE:'realtime',OATHRA_VOICE_MODEL:'gpt-realtime-1.5',OATHRA_CARRIER_JPY_PER_USD:'157.888307',OATHRA_CARRIER_FX_DATE:'2026-09-18',OATHRA_REALTIME_PRICES_JSON:JSON.stringify({model:'gpt-realtime-1.5',version:'ui-boundary',inputText:4,inputAudio:32,cachedText:0.4,cachedAudio:0.4,outputText:16,outputAudio:64}),OATHRA_SETTLEMENT_MODE:'usage-rate-v1',OATHRA_USAGE_PRICES_JSON:JSON.stringify(usagePrices)});
...
try{
 ...
 await page.js("window.confirm=()=>true;document.querySelector('#phone-template').value='chat';document.querySelector('#phone-template').dispatchEvent(new Event('change'))");await page.click('#phone-form button[type=submit]');await page.until("document.querySelector('#phone-review').open");assert.match(await page.text('#phone-dial'),/最大 400/);assert.match(await page.text('#phone-review-fields'),/600秒/);assert.match(await page.text('#phone-review-fields'),/0.01/);assert.match(await page.text('#phone-review-fields'),/157.888307.*2026-09-18/);assert.match(await page.text('#phone-disclosure'),/終了時.*検索.*精算/);
 ...
 for(const e of [{kind:'started'},{kind:'pending',id:'usage-boundary'},{kind:'response',id:'usage-boundary',model:config.billing.model,usage:{input_tokens:1000,output_tokens:100,input_token_details:{text_tokens:800,audio_tokens:200,cached_tokens:200,cached_tokens_details:{text_tokens:100,audio_tokens:100}},output_token_details:{text_tokens:20,audio_tokens:80}}},{kind:'closed',complete:true}])applyBillingEvent(m,{type:'billing.ai',...e});
 ...
 await page.until("document.querySelector('#phone-live-credits').textContent.includes('今回の消費：23')");assert.match(await page.text('#phone-live-credits'),/返却：377/);assert.match(await page.text('#phone-live-cost'),/通話 32秒.*利用額/);await page.js("document.querySelector('#phone-cost-details summary').focus()");await page.press('Enter');assert.equal(await page.js("document.querySelector('#phone-cost-details').open"),true);assert.match(await page.text('#phone-cost-details'),/Twilio.*gpt-realtime-1.5.*検索 1回/s);assert.doesNotMatch(await page.text('#phone-cost-details'),/暫定/);assert.match(await page.text('#phone-cost-details'),/0.22699|0.200000/);
 ...
 await page.goto(base);await page.until("!document.querySelector('#screen-real').hidden");await page.click('#phone-history-section summary');await page.until("document.querySelector('.phone-history-credits')?.textContent.includes('23')");assert.match(await page.text('#phone-history-list'),/Twilio.*gpt-realtime-1.5/s);
 ...
 for(const amount of [20,0]){
  ...
  await page.click('#phone-form button[type=submit]');await page.until("document.querySelector('#phone-review').open");assert.equal(await page.js("document.querySelector('#phone-dial').disabled"),true);assert.match(await page.text('#phone-dial-hint'),/最低 21.*利用可能 20/);
 ...
 console.log('PASS: leftover377 can reserve377 and cancel/refund; held credits and minimum21 vs20/zero have visible reasons; no calls');
