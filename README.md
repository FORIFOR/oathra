...
`npx oathra demo` で「自分が電話に出る」を選び、店員役として次の 3 つを言ってみてください。どれも「確定」に ✓ が付かないはずです（[検証記録](docs/launch/miscompletion-cases.md)）。

- 「たぶん大丈夫ですが、まだ確定ではありません」→ 確定にならない
- 「19時は満席です。19時半なら空いています」→ 19時は予約時刻として採用されず、19時半は受諾されるまで未確定
- 「ご予約承りました」の後に「料金は2万3500円になります」→ 確定が失効し、取り直しが必要になる
