...
Oathra は電話相手の属性を推測したり、目的外の情報を密かに集めたりしません。現在の対話は `CallContract` に書いた目的・必須項目だけを質問し、同じ質問の反復を抑えます。決定事項は `result.json`、発話に紐づいた `evidence`、`transcript.json` に記録します。追加のプロフィール項目を扱う場合は、収集目的・項目・質問上限・同意・拒否時の停止を契約に明示する拡張が必要です。
