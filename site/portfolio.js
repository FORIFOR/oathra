(() => {
 ...
 let simStarted=false,simComplete=false;
 document.querySelector('#sim')?.addEventListener('click',e=>{if(!e.target.closest('.chip:not(.reset)'))return;if(!simStarted){simStarted=true;event('demo_start')}if(!simComplete&&document.querySelector('#sim .res.ok')){simComplete=true;event('demo_complete')}});
