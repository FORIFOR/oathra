...
if (root) {
  ...
  // A specific time, so the shop restating it settles it and the only open field is the one the call was about.
  const opening = say('9月12日の19時半に2名で予約をお願いします。', 'Please book a table for two on September 12 at 7:30 pm.');
  const options = [
    say('たぶん大丈夫ですが、まだ確定ではありません。', "Probably fine, but it is not confirmed yet."),
    say('あいにく19時半は満席です。19時なら空いています。', "7:30 pm is full, I'm afraid. 7 pm is available."),
    say('はい、9月12日19時半に2名様でご予約承りました。', 'Yes, you are booked for two on September 12 at 7:30 pm.'),
  ...
  root.innerHTML = `<div class="ph"><span><b>${say('予約の証拠ラボ', 'Reservation evidence lab')}</b></span><span class="tag">${say('実エンジン · 発信なし', 'REAL ENGINE · NO PHONE CALL')}</span></div>
    ...
    <aside><div><h4>${say('ミッション', 'MISSION')} <b id="s-n"></b></h4><ul id="s-m"></ul><p class="res" id="s-res" role="status" aria-live="polite"></p></div><div><h4>${say('発言に紐づく証拠', 'EVIDENCE FROM UTTERANCES')}</h4><div class="ev" id="s-ev"></div></div></aside></div>
    <p class="note">${say('本体と同じ判定コードをブラウザで実行。入力は送信されません。音声認識・AIとの通話ではなく、文字からの証拠判定を試す機能です。日付の基準年：2026。', 'The production evidence engine runs in your browser. Your text stays here. This checks transcript evidence, not speech recognition or an AI call. Date reference year: 2026.')}</p>`;
  ...
  function render() {
    ...
    // One sentence usually settles several fields at once. Quoting it three times reads as three findings,
    // so the fields are grouped under the utterance they came from.
    const quotes = new Map<string, { source: string; lines: string[]; note?: string }>();
    for (const f of fields) {
      const node = result.fields[f] !== undefined ? engine.latestVerified(f) : engine.pending(f);
      if (!node) continue;
      const entry = quotes.get(node.transcript) ?? { source: node.source, lines: [] };
      entry.lines.push(`${label(f)}: ${value(node.value)}`);
      if (node.value === null && f === 'time') entry.note = say('午前・午後を明示して、時刻を再確認してください。', 'Specify am/pm and confirm the time again.');
      quotes.set(node.transcript, entry);
    }
    for (const [transcript, entry] of quotes) {
      const d = document.createElement('div'); d.className = entry.source === 'callee' ? 'callee' : 'ai';
      d.textContent = `${entry.lines.join(' · ')} · ${entry.source}\n“${transcript}”${entry.note ? '\n' + entry.note : ''}`;
      el('s-ev').append(d);
    }
  ...
  function resetEngine() { engine = new EvidenceEngine({ language: ja ? 'ja' : 'en', now: new Date('2026-09-01T00:00:00Z') }); seq = 0; el('s-tx').replaceChildren(); el<HTMLTextAreaElement>('evidence-text').value = ''; ingest('caller', opening); }

  /**
   * The panel opens on the disagreement rather than on an empty form: the agent has already declared
   * success, the shop's own words have settled the date, the time and the party — and the one field the
   * call was about is still open, because the shop is still checking. Nothing here is precomputed; these
   * lines run through the same engine as everything the visitor types afterwards.
   */
  const seed: [Speaker, string][] = [
    ['callee', say('かしこまりました、9月12日19時半に2名様ですね。ただいま空き状況を確認しております。', 'Certainly — September 12, 7:30 pm, two people. I am checking availability now.')],
    ['caller', say('ご予約承りました。ありがとうございました。', 'Your reservation is confirmed. Thank you.')],
  ];
  function reset() {
    resetEngine();
    for (const [source, text] of seed) ingest(source, text);
    // The opening state is meant to be read from the top, not joined mid-conversation.
    el('s-tx').scrollTop = 0;
  }
