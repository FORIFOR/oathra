...
Oathra v0.1.16では、電話会社・音声モデル・エージェント基盤を移行せず、保存済みの文字起こしを検査できます。仮押さえ・未確定・承認待ちを予約完了と扱わない判定に加え、会話から結果までを証拠レベルとして記録する `ActionProof` を提供します。判定はローカルで動き、API呼び出し・APIキーは不要です。[English / 詳細な入力仕様](INTEGRATION.md)
