...
The public v0.1.6 package is currently distributed from the GitHub Release because the local npm session is not authenticated. The repository now includes a manual GitHub Actions path so a maintainer only needs to configure the credential once and select the tag to publish.
...
3. Open **Actions → Publish npm package → Run workflow** and enter the exact Git tag, such as `v0.1.6`.
