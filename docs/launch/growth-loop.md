...
### Current product checkpoint — v0.1.6

The user-requested follow-up workflow is now implemented as consent-based, contract-declared intake. After the required call details settle, Oathra asks once for permission, asks at most one declared field per turn (default cap three, hard cap eight), stops on decline, and saves only the next explicit callee answer with its utterance ID and timestamp. `intake.json` and `summary.md` make the collected answers and decisions reviewable. The feature does not infer a profile, collect undeclared or sensitive attributes, or continue after refusal. Release: https://github.com/FORIFOR/oathra/releases/tag/v0.1.6.

The 2026-09-14 13:57 JST readback remains 0 stars / 0 forks, 6 views / 6 unique visitors and 298 clones / 121 unique cloners in GitHub's 14-day window; v0.1.6 assets had 0 downloads at that snapshot. These are distribution signals, not confirmed users. Zenn likes are 1 for `oathra-launch` and 0 for `oathra-evidence-rules`; no external feedback or business inquiry has been observed.
