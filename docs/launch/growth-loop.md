...
The user-requested follow-up workflow is now implemented as consent-based, contract-declared intake. After the required call details settle, Oathra asks once for permission, asks at most one declared field per turn (default cap three, hard cap eight), stops on decline, hold or an ambiguous reply, and saves only the next explicit callee answer with its utterance ID and timestamp. `intake.json` and `summary.md` make the collected answers and decisions reviewable. Scenario YAML now carries the same contract into `oathra call`, so local and phone runs share the rule set. The feature does not infer a profile, collect undeclared or sensitive attributes, or continue after refusal. The v0.1.10 asset also includes the built-in Arena intake mission and both language recordings: https://github.com/FORIFOR/oathra/releases/tag/v0.1.10.

The latest 2026-09-14 17:12 JST readback remains 0 stars / 0 forks, 6 views / 6 unique visitors and 298 clones / 121 unique cloners in GitHub's 14-day window. v0.1.10's tarball and checksum each show one download, which is the maintainer's release verification and not adoption. These are distribution signals, not confirmed users. Zenn likes are 1 for `oathra-launch` and 0 for `oathra-evidence-rules`; GitHub Discussion #14 has two maintainer comments and no external replies, and no business inquiry has been observed. The raw snapshot is [2026-09-14-1712-jst.json](metrics/2026-09-14-1712-jst.json).

The existing [awesome-voice-agents#42](https://github.com/yzfly/awesome-voice-agents/pull/42) listing was updated in place at 17:10 JST to describe v0.1.10's consented intake and link the 48-second demo. This is a distribution experiment for the selected voice-agent audience; no maintainer response or adoption is claimed.

The Japanese and English Pages routes contain the v0.1.10 intake recording, scenario link and private inquiry form. A CORS preflight to the form's leads and events endpoints returned 204 for the Pages origin; no inquiry was submitted during verification.

Current X publication text: [x-transcript-check.txt](x-transcript-check.txt). This is the single active draft; older drafts in the publication history are superseded. A fresh public timeline read at 2026-09-14 16:54 JST showed newer account activity, with the newest post displayed as 1h (status `2099380959384899784`), followed by posts displayed as 2h and 14h. Its public status metadata reports `2026-09-14T06:13:19Z` (`2026-09-14 15:13:19 JST`), so defer the Oathra announcement until 2026-09-15 15:13:19 JST or later and recheck the live timeline immediately before publication. This supersedes the earlier 16:54 JST and 03:01 JST estimates. Do not classify a repost as a new original announcement, or invent outside engagement from the author's own activity.
