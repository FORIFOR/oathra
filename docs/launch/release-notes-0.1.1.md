...
- **敵対的スイートを 5 種 → 10 種に拡張**: 仮押さえ（一応・後で確認します）、聞き返し（…でよろしいでしょうか？）、確定直後の条件訂正、確定直後の取り消し、保留。拡張で見つかった 2 つの穴を塞いだ。店員の「合っておりますでしょうか？」が同意扱いだった点と、「承りました。……やはりお取りできませんでした」が確定扱いだった点。
...
- Adversarial suite grown from 5 to 10 mutations (tentative hold, asking back, confirm-then-change, confirm-then-retract, hold). Two gaps it exposed are closed: a callee asking "…is that right?" counted as agreement, and "booked … actually we can't" counted as confirmed.
