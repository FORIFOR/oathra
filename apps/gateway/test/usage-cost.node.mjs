...
const env={OATHRA_CREDIT_POLICY:'provider-cost-v1',OATHRA_CREDIT_USD:'0.01',OATHRA_VOICE_ENGINE:'gpt-live',OATHRA_VOICE_MODEL:'gpt-live-1',OATHRA_LIVE_PRICES_JSON:JSON.stringify({model:'gpt-live-1',version:'unit-boundary',perMinute:'0.01152'}),OATHRA_SETTLEMENT_MODE:USAGE_RATE,OATHRA_USAGE_PRICES_JSON:JSON.stringify({version:'unit-boundary',carrier:[{prefix:'+81',currency:'USD',perMinute:'0.1',incrementSeconds:60,source:'bounded test rate'},{prefix:'+8190',currency:'USD',perMinute:'0.2',incrementSeconds:60,source:'bounded test rate'}],mediaPerMinute:'0.0044',search:{model:'gpt-5.4-mini',perCall:'0.01',input:'0.75',cached:'0.075',output:'4.5'}})};
function fixture(){
 ...
 // A 32 second media stream is one started voice minute: 11,520,000 nanodollars.
 applyBillingEvent(m,{type:'billing.timing',startedAt:1000,endedAt:33000});
 m.billing.connectedAt=1000;m.billing.executionFinished=true;store.put('mission',m);
...
test('ends without carrier price: minute rounding, voice minutes, search and one final credit rounding',using(f=>{
 ...
 assert.equal(f.credits.usage(m).cost.voiceModel,'gpt-live-1');assert.equal(f.credits.usage(m).cost.quantities.voiceSeconds,32);assert.equal(f.credits.usage(m).cost.searchCalls,1);
}));
test('longer calls and additional searches increase cost; a second started minute doubles the voice charge',using(({m})=>{
 const a=meteredCost(m);m.billing.timing.endedAt=62000;const b=meteredCost(m);assert.equal(b.carrierNanoUsd,400000000);assert.equal(a.aiNanoUsd,11520000);assert.equal(b.aiNanoUsd,23040000);assert.ok(b.totalNanoUsd>a.totalNanoUsd);search(m);assert.equal(meteredCost(m).totalNanoUsd-b.totalNanoUsd,11065000);
}));
...
test('restart after a definitive rejected dial returns the entire hold with no carrier SID',using(async f=>{
 f.m.status='FAILED';f.m.error='carrier_dial_rejected';f.m.billing={state:'pending'};f.store.put('mission',f.m);
 const worker=new Worker({store:f.store,credits:f.credits,notify(){}},{},undefined);worker.start();
...
test('identical usage, repeated settlement, late invoice, restart and tariff change never double debit',using(f=>{
 ...
 f.m.billing.carrier={durationSeconds:100,costNanoUsd:999999999};f.config.billing.voicePerMinuteNanoUsd=999999999;
...
test('unknown side effects keep hold; an unmeasured session is explicitly absorbed after a known stop',using(f=>{
 ...
 // The carrier reported its duration, but the media-stream timing was lost.
 delete m.billing.timing;m.billing.carrier={durationSeconds:32,costNanoUsd:null,amount:null};applyBillingEvent(m,{type:'billing.search',id:'lost',kind:'started',model:'gpt-5.4-mini'});
 const c=meteredCost(m);assert.deepEqual(c.excluded,['unmeasured-media-usage','unmeasured-voice-usage','unmeasured-search-usage']);assert.equal(c.aiNanoUsd,0);assert.equal(c.mediaNanoUsd,0);assert.equal(c.searchNanoUsd,0);
}));
...
test('failed call before connection is free, upper bound cannot overdraw, settlement rolls back atomically',using(f=>{
 const m=f.m;m.billing.timing=undefined;m.billing.carrier={durationSeconds:0,costNanoUsd:0};delete m.billing.connectedAt;assert.equal(meteredCost(m).totalNanoUsd,0);
...
test('signed Twilio callback authenticates identity, records duration and is idempotent',using(f=>{
 ...
 f.m.carrierSid=sid;f.store.put('mission',f.m);f.store.setKey('optout',token,f.store.seal({mission:f.m.id}));
 ...
 assert.equal(f.credits.usage(f.m).consumed,22);assert.equal(f.credits.usage(f.m).cost.durationSource,'twilio-duration');assert.equal(f.credits.history(f.m.owner).filter(e=>e.kind==='consume').length,1);
