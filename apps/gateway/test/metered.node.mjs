...
import {METERED,billingConfiguration,responseCost,applyBillingEvent,meteredCost} from '../lib/billing.mjs';
import {Phone} from '../lib/phone.mjs';
import {createServer} from 'node:http';
const prices={model:'gpt-realtime-1.5',version:'2026-09-20',inputText:4,inputAudio:32,cachedText:0.4,cachedAudio:0.4,outputText:16,outputAudio:64};
const billing=billingConfiguration({OATHRA_CREDIT_POLICY:METERED,OATHRA_CREDIT_USD:'0.01',OATHRA_VOICE_ENGINE:'realtime',OATHRA_VOICE_MODEL:prices.model,OATHRA_REALTIME_PRICES_JSON:JSON.stringify(prices)});
const usage={input_tokens:1000,output_tokens:100,input_token_details:{text_tokens:800,audio_tokens:200,cached_tokens:200,cached_tokens_details:{text_tokens:100,audio_tokens:100}},output_token_details:{text_tokens:20,audio_tokens:80}};
function setup(){
...
function finish(f,carrier=10000000){
 const m=f.m;for(const e of [{kind:'started'},{kind:'pending',id:'r1'},{kind:'response',id:'r1',model:prices.model,usage},{kind:'closed',complete:true}])applyBillingEvent(m,{type:'billing.ai',...e});
 m.billing.carrier={costNanoUsd:carrier,durationSeconds:25};m.billing.executionFinished=true;f.store.put('mission',m);return m;
}
test('cached text/audio rates use provider quantities and malformed usage stays unknown',()=>{
 assert.equal(responseCost(usage,billing.rates),11520000);
 for(const u of [undefined,{}, {...usage,input_tokens:1001},{...usage,input_token_details:{...usage.input_token_details,cached_tokens_details:{text_tokens:300,audio_tokens:0}}},{...usage,output_tokens:NaN}])assert.equal(responseCost(u,billing.rates),null);
});
...
test('missing, conflicting or interrupted usage cannot be charged as zero',using(f=>{
 const m=finish(f);m.billing.ai.complete=false;assert.equal(meteredCost(m),null);assert.equal(f.store.tx(()=>f.credits.settleTx(m)),false);
 m.billing.ai.complete=true;applyBillingEvent(m,{type:'billing.ai',kind:'response',id:'r1',model:prices.model,usage:{}});assert.equal(meteredCost(m),null);
 assert.equal(f.credits.balance(m.owner).held,400);
}));
test('duplicate usage is not added; a different tariff after approval cannot reprice a call',using(f=>{
 const m=finish(f);applyBillingEvent(m,{type:'billing.ai',kind:'response',id:'r1',model:prices.model,usage});assert.equal(m.billing.ai.responses.length,1);
...
test('unsettled costs survive retention; zero cost returns the full hold',using(f=>{
 const m=finish(f,0);m.billing.ai={started:false,closed:false,complete:false,pending:[],responses:[]};m.finishedAt=1;f.store.put('mission',m);
...
test('only an administrator can absorb unmeasurable costs after carrier reconciliation',using(f=>{
 const m=finish(f);m.billing.ai.complete=false;f.store.put('mission',m);
...
test('restart marks local execution finished while retaining incomplete usage and all pending holds',using(async f=>{
 const {Worker}=await import('../lib/worker.mjs'),{Service}=await import('../lib/service.mjs');
 const m=finish(f);m.status='DIALING';delete m.billing.executionFinished;m.billing.ai.closed=false;f.store.put('mission',m);
 ...
 worker.start();try{const restored=f.store.get('mission',m.id);assert.equal(restored.status,'UNKNOWN');assert.equal(restored.billing.executionFinished,true);assert.equal(restored.billing.ai.complete,false);assert.equal(f.credits.pendingMissions().length,1)}finally{await worker.stop()}
}));

test('synchronous engine close on an opt-out cannot overwrite persisted usage',using(async f=>{
 const {Worker}=await import('../lib/worker.mjs'),{Service}=await import('../lib/service.mjs');
 ...
 m.billing={state:'pending',ai:{started:true,closed:false,complete:false,pending:[],responses:[]}};f.store.put('mission',m);
 const worker=new Worker(new Service(f.store,f.config),{},async(_m,h)=>{h.signal.addEventListener('abort',()=>h.onEvent({type:'billing.ai',kind:'closed',model:prices.model,complete:true}));h.onEvent({type:'contact.opt_out'});return {transcript:[]}});
 await worker.run(m,{abort:new AbortController(),control:{}});
 assert.equal(f.store.get('mission',m.id).billing.ai.closed,true);assert.equal(f.store.get('mission',m.id).billing.ai.complete,true);
}));
...
test('restart after a terminal result write still resumes pending price reconciliation',using(async f=>{
 ...
 const worker=new Worker(new Service(f.store,f.config),{},undefined);worker.start();try{const saved=f.store.get('mission',m.id);assert.equal(saved.status,'FAILED');assert.equal(saved.billing.executionFinished,true);assert.equal(saved.billing.ai.complete,true)}finally{await worker.stop()}
}));
