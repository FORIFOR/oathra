...
`OATHRA_INBOUND_OWNER`（受け手の利用者ID）と `OATHRA_INBOUND_NAME`（誰の電話として出るか）を設定し、Twilioの番号の着信Webhookを `POST <公開URL>/hooks/twilio/voice` に向けると、かかってきた電話にAIが出ます（`node --env-file=<env> apps/gateway/inbound-setup.mjs` で現在値の確認、`--apply` で設定、`--restore <url>` で復元）。未設定なら着信機能は無効で、番号の設定も変えません。

- **だれの通話か**: この番号から30日以内にこちらがかけた相手からの着信は、その電話を依頼した利用者の通話になり、AIは「〇〇さんの代わりに先日お電話した件の折り返し」として用件を把握します（依頼文は参考情報としてだけ渡し、読み上げません）。それ以外は `OATHRA_INBOUND_OWNER` の通話です。
- **AIが出る条件**: 署名が正しい／発信者番号が分かる／受け手が同意済み／連絡停止の番号でない／同じ番号から1時間に `OATHRA_INBOUND_PER_CALLER_PER_HOUR` 回、全体で `OATHRA_INBOUND_PER_HOUR` 回以内／他の通話中でない／受け手の残高で最初の1分を確保できる／着信の回線単価（`OATHRA_USAGE_PRICES_JSON.inbound`）が設定済み。一つでも欠ければ、日本語で「AIによる代理電話サービスの発信用の番号であること」（折り返しなら誰の依頼だったか）を案内して切ります。このとき課金はなく、`call.inbound_not_answered` を理由つきで監査ログに残します。案内中に2を押すと、その番号への今後の発信を停止します。
- **応対の範囲**: 冒頭に「この通話は記録されています。」、続けてAIが「〇〇さんの電話を預かっているAIアシスタントです」と名乗ります。聞き取るのは名前・用件・折り返しの要否と時間だけで、折り返しや対応の約束はしません。受け手の予定・居場所・連絡先・過去の通話内容は答えず、予約・購入・支払い・契約・認証番号の提供も行いません。検索も行いません。緊急の内容には緊急連絡先への直接の連絡を促します。
- **店の予約受付として出る（任意）**: `OATHRA_RESTAURANT_JSON`（店名・時刻ごとの卓数・1組の上限・休業日）を設定すると、着信はすべて店の予約受付になります（`OATHRA_INBOUND_NAME` は不要。通話は `OATHRA_INBOUND_OWNER` の負担）。AIは話すだけで、空席の確認と予約の記録は台帳（`@oathra/core` の reservation desk）が決めます。
  - 空席は `check_table`、記録は `book_table` だけ。`book_table` が `booked` を返すまで「承りました」とは言わせません。
  - 復唱ゲート: 日付・時刻・人数・名前をAIがその通話で声に出していなければ、`book_table` は `read_back_required` で拒否されます（判定は決定的パーサー。モデルの自己申告ではありません）。
  - 1通話1予約: 同じ通話で言い直せば同じ予約、内容を変えれば付け替え（元の卓は空きに戻る）。確認と書き込みは1トランザクションなので、最後の1卓を2本の電話に渡すことはありません。
  - 記録は画面の「予約台帳」（店の利用者のヘッダーにだけ出ます。日ごとの時刻・残り卓数・予約名、名前検索、予約を受けた通話へのリンク）、`GET /v1/phone/bookings`（台帳の設定とこれからの予約。店の利用者以外には空）、各通話の結果の `booking`。相手の電話番号はモデルにも一覧にも渡しません。予約日から30日で削除します。
  - 音声なしの突き合わせ試験: `node --env-file=.env scripts/reception-sim.mjs`（発信AIと受付を同じ通話で走らせ、発信側の `evaluate()` の判定と台帳を12状況で比較。OpenAI APIを数円使用、発信なし）。実際の音声通話での確認は別途必要です。
- **費用と上限**: 受け手のクレジットから、発信と同じ方式（残高の範囲で確保→使った分だけ消費→残りを返却、上限到達で自動終了）で精算します。回線は着信単価、音声AIと中継は発信と同じ単価です。1通話は `OATHRA_INBOUND_MAX_SECONDS`（既定180秒）まで。かけてきた人に本サービスからの課金はありません。
- **記録**: 履歴に「着信：」として残り、文字起こし・通話メモ・利用内訳は発信と同じ画面で確認できます。

番号が海外のもの（例: 米国の +1）の場合、日本からかけ直す人には国際電話になります。国内からの折り返しを想定するなら日本の番号が必要です。一時トンネルの公開URLは再起動のたびに変わるので、そのつどWebhookの再設定が必要です。
