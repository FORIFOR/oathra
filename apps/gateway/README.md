...
ブラウザで `http://localhost:4244` を開き、`.oathra/operator-token.txt` の内容でログインします。「準備・設定」でデータ利用を確認し、デモ商品・デモ担当者を選んで依頼します。確認画面で承認した場合だけ模擬実行します。例は「資料送付の了承はあるが、商談日時は未確定」という結果です。
...
本文・イベントは暗号化しますが、レコードID・所有者ID・状態などの索引メタデータは平文です。完了した会話記録は標準30日、本文を含まない監査記録は90日で削除します。未確定の回線状態は照合まで保持し、再連絡停止は別記録として残します。外部サービスに送信済みの内容はこのDBの削除だけでは消えません。
