...
(function () {
  ...
  const STR = {
    en: {
      ...
      phoneEngine: "Voice AI", phoneEngineNote: "Which model speaks on the call. Only engines whose API key is set on this server can be chosen; the voice is fixed for the whole call.", phoneEngineUnavailable: "not set up", phoneNumber: "Recipient phone number", phoneName: "Recipient name", phoneCallerName: "Your name (optional, told to callee)", phoneInstruction: "Purpose", phoneHelp: "Japanese numbers may start with 0. For other countries, include + and the country code. One number only; no extension.",
    ...
    ja: {
      ...
      phoneEngine: "音声AI", phoneEngineNote: "通話で話すモデルです。このサーバーに API キーがある音声AIだけ選べます。通話の途中では変えられません。", phoneEngineUnavailable: "未設定", phoneNumber: "電話番号", phoneName: "相手", phoneCallerName: "あなたの名前（相手に伝えます・任意）", phoneInstruction: "目的", phoneHelp: "日本の番号は0から入力できます。海外の番号は＋と国番号から入力してください。番号は1件、内線は指定できません。",
  ...
  $("#phone-form").addEventListener("submit", async e => {
    ...
    const input = { phone: $("#phone-number").value, name: $("#phone-name").value, instruction: $("#phone-instruction").value, ...(callerVal ? { callerName: callerVal } : {}), ...(engineVal ? { engine: engineVal } : {}), ...(phoneConversationMode==="chat"?{conversationMode:"chat"}:{}) };
    try {
      ...
      $("#phone-review-fields").replaceChildren(...[["phoneNumber", request.phone], ["phoneName", request.name], ...(request.callerName ? [["phoneCallerName", request.callerName]] : []), ...(request.engine ? [["phoneEngine", request.engine]] : []), ["phoneInstruction", request.instruction]].flatMap(([label, value]) => [el("dt", {text:t(label)}), el("dd", {text:value})]));
